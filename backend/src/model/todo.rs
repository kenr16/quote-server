...
impl TodoMac {
    pub async fn create(db: &Db, utx: &UserCtx, data: TodoPatch) -> Result<Todo, model::Error2> {
    ...
    pub async fn get(db: &Db, _utx: &UserCtx, id: i64) -> Result<Todo, model::Error2> {
    ...
    pub async fn update(db: &Db, utx: &UserCtx, id: i64, data: TodoPatch) -> Result<Todo, model::Error2> {
		let mut fields = data.fields();
    ...
    pub async fn list(db: &Db, _utx: &UserCtx) -> Result<Vec<Todo>, model::Error2> {
    ...
    pub async fn delete(db: &Db, _utx: &UserCtx, id: i64) -> Result<Todo, model::Error2> {
		let sb = sqlb::delete()
			.table(Self::TABLE)
			.returning(Self::COLUMNS)
			.and_where_eq("id", id);
